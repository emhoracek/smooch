...
spec = do
  describe "convertCel" $ do
    it "converts a cel to png" $
      runExceptT (convertCel samplePalettes 0 "aurora" (sampleDir <> "/aurora")) `shouldReturn` Right ("aurora", (0,0))
    it "gives an error if the file isn't there" $
      runExceptT (convertCel samplePalettes 0 "aurora.cel" (sampleDir <> "/aurora")) `shouldReturn`
  ...
  describe "convertCels" $ do
    it "finds the transparent color and converts all the cels" $
      runExceptT (convertCels samplePalettes [fakeCel] (sampleDir <> "/aurora")) `shouldReturn` Right [("aurora", (0,0))]
    it "returns an error if a cel is missing" $
      runExceptT (convertCels samplePalettes [fakeCel { cnfCelName = "aurora.cel"}] (sampleDir <> "/aurora")) `shouldReturn`
        Left "Error while converting cel tests/samples/aurora/aurora.cel.cel. Exit code: 1. Error: Read palette tests/samples/aurora/color.kcf \nNew style palette\nRead cel tests/samples/aurora/aurora.cel.cel \ntests/samples/aurora/aurora.cel.cel: No such file or directory\n"


  describe "transColor" $ do
    it "gets the transparent color from a palette" $
      runExceptT (transColor (sampleDir <> "/aurora/color.kcf")) `shouldReturn` Right "rgb:ff/f7/ff"
    it "returns an error if the palette isn't there" $
      runExceptT (transColor "potato") `shouldReturn`
  ...
  describe "borderColor" $ do
    it "gets the transparent color from a palette" $
      runExceptT (colorByIndex 3 (sampleDir <> "/aurora/color.kcf")) `shouldReturn` Right "#ffff94"
    it "returns an error if the palette isn't there" $
      runExceptT (colorByIndex 15 "potato") `shouldReturn`
